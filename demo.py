...
# Đọc ảnh xám mẫu
img = cv2.imread(cv2.samples.findFile("ex.png"), cv2.IMREAD_GRAYSCALE)

# Thêm nhiễu xung
noisy_img = add_salt_pepper_noise(img, prob=0.1)

# Phát hiện nhiễu
noise_mask = impulse_noise_detector(noisy_img)
...
plt.subplot(1, 4, 3); plt.imshow(noise_mask, cmap='gray'); plt.title("Mặt nạ nhiễu (phát hiện nhiễu)"); plt.axis('off')
plt.subplot(1, 4, 4); plt.imshow(denoised_img, cmap='gray'); plt.title("Ảnh đã lọc nhiễu"); plt.axis('off')
plt.tight_layout()
plt.show()
